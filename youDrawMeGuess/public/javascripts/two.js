...
(function () {
  ...
  //初始化时判断在线人数
  socket.emit('getTwoNum'+num,{
    num:'+'
  });
  socket.on('getTwoNum'+num,function(data){
    console.log(data);
    clearTimeout(time);
    count=data.count;
     if(!order){
...
//聊天室
    socket.on('chatMessage'+num,function(data){
      console.log(data);
...
alert("我监听到了浏览器的返回按钮事件啦");//根据自己的需求实现自己的功能
   ...
   $('#selectColor ul li').click(function(){
     console.log('选择颜色');
      ...
      //   lineWidth='2';
      // }
      socket.emit('changeColor'+num,{
        color:$(this).css('background-color')
      })
   });

    socket.on('changeColor'+num,function(data){
  ...
  //开始游戏或者结束游戏后更换游戏位置
  socket.on('getTitle'+num,function(data){
